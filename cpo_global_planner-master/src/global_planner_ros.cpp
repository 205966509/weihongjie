...
namespace global_motion_planner
{
  ...
  void globalMotionPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
  {
    if (!initialized_)
    {
      ...
      bool pub_particles;
      int n_particles,n_inherited,pointNum,T,initposmode,cpo_max_iter;
      double obs_factor, alpha, Tf, N_min;
      private_nh.param("n_particles", n_particles, 50);        // 种群数量
      private_nh.param("n_inherited", n_inherited, 20);        // 继承粒子
      private_nh.param("pointNum", pointNum, 5);               // 控制点数
      private_nh.param("obs_factor", obs_factor, 0.39);        // 障碍物因子
      private_nh.param("T", T,2);             // 周期
      private_nh.param("alpha", alpha,0.2);          // 收敛率
      private_nh.param("Tf", Tf, 0.8);             // 第三第四比例
      private_nh.param("N_min", N_min, 30.0);       // 最小种群
      private_nh.param("initposmode", initposmode, 2);         // 初始化模式
      private_nh.param("pub_particles", pub_particles, false); // 发布粒子
      private_nh.param("cpo_max_iter", cpo_max_iter, 5);       // 最大迭代次数

      g_planner_ = new CPO(nx_, ny_, resolution_ ,origin_x_,origin_y_, n_particles,n_inherited, pointNum, alpha, Tf, N_min, obs_factor, T ,initposmode ,pub_particles,cpo_max_iter);

      ROS_INFO("CPO planner initialized successfully");
      initialized_ = true;
    }
    else
    {
